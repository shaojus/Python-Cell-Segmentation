...
def CellShapeAutoencoder(FileList, train_frac):
    ...
    h = layers.Conv2D(32, (3, 3), activation="relu", padding="same")(
        input_img
    )  # 128x128x32
    h = layers.MaxPooling2D((2, 2), padding="same")(h)  # 64x64x32
    h = layers.Conv2D(16, (3, 3), activation="relu", padding="same")(h)  # 64x64x16
    h = layers.MaxPooling2D((2, 2), padding="same")(h)  # 32x32x16
    h = layers.Conv2D(8, (3, 3), activation="relu", padding="same")(h)  # 32x32x8
    h = layers.MaxPooling2D((2, 2), padding="same")(h)  # 16x16x8
    h = layers.Conv2D(4, (3, 3), activation="relu", padding="same")(h)  # 16x16x4
    encoded = layers.MaxPooling2D((2, 2), padding="same")(
        h
    )  # 8x8x4 (256 dimensionality)

    h = layers.Conv2D(4, (3, 3), activation="relu", padding="same")(encoded)
    h = layers.UpSampling2D((2, 2))(h)
    h = layers.Conv2D(8, (3, 3), activation="relu", padding="same")(h)
    h = layers.UpSampling2D((2, 2))(h)
    h = layers.Conv2D(16, (3, 3), activation="relu", padding="same")(h)
    h = layers.UpSampling2D((2, 2))(h)
    h = layers.Conv2D(32, (3, 3), activation="relu", padding="same")(h)
    h = layers.UpSampling2D((2, 2))(h)
    decoded = layers.Conv2D(1, (3, 3), activation="sigmoid", padding="same")(h)
