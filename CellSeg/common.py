...
def BBoxCalc(tempobject):
    ...
    properties = measure.regionprops(tempobject)#ideally find a way to only measure bounding box for speed?
...
def imimposemin(I, BW, conn=2):
    ...
    g = np.minimum(fp1,fm)
    if conn == 2:
        J = morphology.reconstruction(util.invert(fm),util.invert(g), selem = morphology.square(3))
    if conn == 1:
        J = morphology.reconstruction(util.invert(fm),util.invert(g), selem = morphology.disk(1))
