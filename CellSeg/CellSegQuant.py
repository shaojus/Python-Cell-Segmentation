...
def blurimg2_batch(nuc):
    ...
    nuc =  np.uint8((nuc.astype(float) - float(np.amin(nuc[:]))) / (float(np.amax(nuc[:])) - float(np.amin(nuc[:]))) * 255)
...
def MaskFiltration(mask, low):
    ...
    filtMask = bwareafilt(mask,[(min(areas)+low*(max([area - min(areas) for area in areas]))), max(areas)])
    return filtMask
...
def ML_probability(probs, LowAreaLim, thresh):
    ...
    tm = cv2.normalize(tm,out,1.0,0.0,cv2.NORM_MINMAX, dtype = cv2.CV_64F) #normalize red channel to grayscale
...
def SegDirFormatting(Directory):
    ...
    if platform.system() == 'Windows':
        ...
        OutDir = [(Seg + '\\' + file + '\\') for file in os.listdir(Seg) if not file.startswith('.')]
    else: 
        AFRemoved = Directory + r'/AFRemoved'
        DAPI = Directory + r'/RegisteredImages/S001'
        Seg = Directory + r'/SegQuants'
        ...
        if not os.path.exists(Seg + r'/CellShape'):
            os.makedirs(Seg + r'/CellShape')
        OutDir = [(Seg + r'/' + file + r'/') for file in os.listdir(Seg) if not file.startswith('.')]
...
def CellSeg(SlideDir, quantify, shape, stroma, tumor, start):
    ...
    (AFRemoved, DAPI, OutDir) = SegDirFormatting(SlideDir); #get formatting for AfRemoved, DAPI, and output directories 
    ...
    for i in (start, range(len(PosList))):
        ...
        if not os.path.exists(OutDir[10] + OutPos[i] + '_stack.tif'):
            ...
            for j in range(len(AFList)): #loop through AFRemoved images and append to tiff stack 
                skimage.external.tifffile.imsave(OutDir[10]+OutPos[i]+'_stack.tif',tiff.imread(AFRemoved+'/'+AFList[j]+'_AFRemoved_'+OutPos[i]+'.tif'), 
                                                 append=True)
        #Check for probability files 
        if not os.path.exists(OutDir[4]+'epi_'+OutPos[i]+'_stack_Probabilities.png'):
            print('No Epithelial Probability File')
            continue
        if not os.path.exists(OutDir[4]+'mem_'+OutPos[i]+'_stack_Probabilities.png'):
            print('No Membrane/Nucleus Probabilty File')
            continue
        
        #nuclear segmentation and generate supermembrane and binary membrane mask 
        if not (os.path.exists(OutDir[7]+'NucMask_'+OutPos[i]+'.png')
            ...
            or os.path.exists(OutDir[5]+'MemMask_'+OutPos[i]+'.png')):
                ...
                Probs = io.imread(OutDir[4]+'mem_'+OutPos[i]+'_stack_Probabilities.png')

                #threshold with nuclear probability >0.6 for nuclear mask
                mask = np.where(Probs[:,:,1]>255*.6, np.uint8(255), np.uint8(0)) 
                ...
                MemMask = np.where(Probs[:,:,0]>255*.6, np.uint8(255), np.uint8(0))
                io.imsave(OutDir[5]+'MemMask_'+OutPos[i]+'.png',
                                  MemMask)
        else:
        ...
        if not (os.path.exists(OutDir[3]+'EpiMask_'+OutPos[i]+'.png')):
            ...
            epiMask = io.imread(OutDir[4]+'epi_'+OutPos[i]+'_stack_Probabilities.png')
            epiMask = ML_probability(epiMask, pixadj*0.01, .45) #create epi mask from probability map
            io.imsave(OutDir[3] + 'EpiMask_' + OutPos[i]+'.png',255*np.array(epiMask, dtype = np.uint8))
        else:
            epiMask = np.array(io.imread(OutDir[3] + 'EpiMask_' + OutPos[i]+'.png'), dtype=bool)
        ...
        if (not os.path.exists(OutDir[8]+'NucMaskFinal_'+OutPos[i]+'.png')) or (not os.path.exists(OutDir[1]+'CellSegFinal_'+OutPos[i]+'.tif')):
                ...
                if not (os.path.exists(OutDir[0]+'CellSeg_'+OutPos[i]+'.tif')):
                    MemMask = np.array(io.imread(OutDir[5]+'MemMask_'+OutPos[i]+'.png'), dtype = bool)
                    start = time.time()
                    CellSeg = ReSegCells(L2, MemMask)
                    end = time.time()
                    print(end - start)
                    io.imsave(OutDir[0]+'CellSeg_'+OutPos[i]+'.tif', np.array(CellSeg, dtype = np.int16))
                else:
                ...
                if not (os.path.exists(OutDir[1] + 'CellSegFinal_' + OutPos[i] + '.tif')):
                    ...
                    (watcellseg, mask) = NucCountBatch(CellSeg, mask, epiMask, MemMask, [], Probs[:,:,1], SuperMem)
                    ...
                    #fill small crosses
                    hitmiss = ndimage.morphology.binary_hit_or_miss(watcellseg, ~filt, np.zeros((5,5)))
                    ...
                    io.imsave(OutDir[1]+'CellSegFinal_'+OutPos[i]+'.tif', np.array(watcellseg, dtype = np.uint16))
                    io.imsave(OutDir[8]+'NucMaskFinal_'+OutPos[i]+'.png', np.array(255*(mask > 0), dtype = np.uint8))
        ...
        if quantify == 1:
            ...
            if os.path.exists(OutDir[9]+'PosStats_'+OutPos[i]+'.csv') and os.path.exists(OutDir[6]+'Novlp_'+OutPos[i]+'.png'):
                continue
            else:
                ...
                if tumor == 1:
                    if not os.path.exists(OutDir[4] + 'TumorMask_' + OutPos[i] + '.png'):
                        tumorMask = io.imread(OutDir[4] + 'tum_' + OutPos[i] + '_stack_Probabilities.png')
                        tumorMask = ML_probability(tumorMask, pixadj * 0.01, .5)
                        io.imsave(OutDir[4] + 'TumorMask_' + OutPos[i] + '.png', np.array(255 * (tumorMask > 0), np.uint8))
                        (Stats, NoOvlp) = MxIF_quantify(i, watcellseg, AFRemoved, AFList, PosList, mask, MemMask, OutPos, tumorMask)
                
                #format data table and write
                transposed_data = list(zip_longest(*Stats.values()))
                with open(OutDir[9] + 'PosStats_' + OutPos[i] + '.csv', 'w', newline = '') as f:
                    writer = csv.writer(f)
        ...
        if stroma == 1:
            print('Stromal quant:')
            if not os.path.exists(OutDir[4] + 'str_' + OutPos[i] + '_stack_Probabilities.png'):
                ('No epithelial probability file')
            elif not os.path.exists(OutDir[9] + 'StrPosStats_' + OutPos[i] + '.csv') or not os.path.exists(OutDir[6] + 'StrNovlp' + OutPos[i] + '.png'):
                stromal_nuclei = stromal_nuclei_segmentation(io.imread(OutDir[4] + 'str_' + OutPos[i] + '_stack_Probabilities.png'))
                stromal_nuclei[epiMask == 1] = 0 
                stromal_grow = morphology.binary_dilation(stromal_nuclei, morphology.square(5))#dilate nuclei 
                #watershed on dilated cells with nuclei as seed points
                stromal_label = morphology.watershed(imimposemin(np.array(stromal_grow, np.uint8), stromal_nuclei), watershed_line = True)
                stromal_label[stromal_grow == 0] = 0 
                io.imsave(OutDir[1]+'StrCellSegFinal_' + OutPos[i] + '.tif', np.array(stromal_label, np.uint16))
                io.imsave(OutDir[8] + 'StrNucMaskFinal_' + OutPos[i] + '.png', 255 * np.array(stromal_nuclei, np.uint8))
                ...
                with open(OutDir[9] + 'strPosStats_' + OutPos[i] + '.csv', 'w', newline = '') as f:
                    writer = csv.writer(f)
        ...
        if shape == 1:
            ...
            if os.path.exists(OutDir[1] + 'CellSegFinal_' + OutPos[i] + '.tif') and not os.path.exists(OutDir[2] + 'CellShape_' + OutPos[i] + '.npz'):
                print('Cell Shape Pre-Processing; ')
                CellImages = tiff.imread(OutDir[1] + 'CellSegFinal_' + OutPos[i] + '.tif')
